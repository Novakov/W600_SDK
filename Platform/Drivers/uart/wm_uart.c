...
/* ČçąűAPBĘ±ÖÓĘÇ40MHzŁ¬ */
/* ˛¨ĚŘÂĘĽÄ´ćĆ÷µÄÖµÉčÖĂÎŞ 115200 : 21 */
/* 9600bps : 260 */
    {
        bd = (apbclk / (16 * 115200) -
...
// //˛»ÄÜÔÚŐâŔďĐŢ¸ÄŁ¬ÎŞÁËĹäşĎÍ¸´«şÍATÖ¸ÁîŁ¬ČíĽţ»á×ÔĽşĐŢ¸ÄflowctrlĹäÖĂŁ¬µ«ĘÇ˛ÎĘý»ąĘÇąĚ¶¨˛»±äµÄ
...
void tls_uart_set_fc_status(int uart_no, TLS_UART_FLOW_CTRL_MODE_T status)
{
    ...
    if (TLS_UART_FLOW_CTRL_HARDWARE == port->opts.flow_ctrl && 0 == status && port->hw_stopped) // ×Ľ±¸ąŘ±ŐÁ÷żŘĘ±Ł¬·˘ĎÖtxŇŃľ­ÍŁÖąŁ¬ĐčŇŞÔŮ´ňżŞtx
    {
...
/*    if (port->uart_mode == TLS_UART_MODE_INT) */// Ä¬ČĎĘąÓĂÖĐ¶ĎµÄ·˝Ę˝
    {
    /* clear interrupt */
...
void tls_set_uart_rx_status(int uart_no, int status)
{
    ...
    // rxstatus=%d\n",uart_no,status,port->rxstatus);
        ...
        // flowctrl=%d,fcstatus=%d\n",port->opts.flow_ctrl,port->fcStatus);
            ...
            {
                cpu_sr = tls_os_set_critical();
                // ąŘrxfifo trigger level interruptşÍoverrun error
        ...
        while (rx_fifocnt-- > 0)
        {
            ...
            if (CIRC_SPACE(recv->head, recv->tail, TLS_UART_RX_BUF_SIZE) <= 2)
            {
                ...
                if (TLS_UART_FLOW_CTRL_HARDWARE == port->fcStatus)
                {
                    ...
                    rx_fifocnt = 0; // ČçąűÓĐÓ˛ĽţÁ÷żŘŁ¬ąŘ±Ő˝ÓĘŐŁ¬°Ń×îşóŇ»¸ö×Ö·ű·Ĺ˝ř»·ĐÎbufferÖĐ
                }
                else
                    break;
            }
...
static s16 tls_uart_tx_cb(struct tls_uart_port *port)
{
    ...
    if (NULL == port)
        return WM_FAILED;
    if (port->buf_len > 0)      // uart1,»şłĺÇřĘýľÝ´«ĘäÍęłÉÖ®şóŁ¬ĘŁÓŕµÄ´ý´«µÄĘýľÝÔŮ·ĹČë»şłĺÇř
    {
